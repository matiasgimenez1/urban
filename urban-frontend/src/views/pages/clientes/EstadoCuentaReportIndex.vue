            ...
            <IndexAndReportFiltersGeneric @apply-filters="handleFilters" @clear-filters="handleFilters" class="headerFilter">
                <template #additional-filters="slotProps">
                    <div>
                        ...
                        <Dropdown inputClass="text-xs font-normal" filter v-model="centrosCostosSelected" :options="clienteStore.clientes" optionLabel="combined" optionValue="code" placeholder="Seleccione un cliente" class="w-full md:w-20rem">
                            <template #option="slotProps">
                                <div class="not-bold text-xs">{{ slotProps.option.combined }}</div>
                            </template>
                        </Dropdown>
                    </div>
                </template>
        ...
        <div class="invoice-wrapper">
            <div id="invoice-content">
                <table class="invoice-table container mx-auto p-4 border-collapse">
                    ...
                    <thead class="bordered-header py-2">
                    ...
                    <tbody v-for="groupKey in Object.keys(groupedData)" :key="groupKey" class="avoid-page-break roboto-mono">
                        ...
                        <tr>
                            <td colspan="9" class="text-lg pt-2 py-2 bg-white roboto-mono">
                                <strong>Cliente:</strong> {{ groupKey.split('***')[0] }} <strong> - Nombre:</strong> {{ groupKey.split('***')[1] }}<strong> - CI/R.U.C.:</strong> {{ groupKey.split('***')[2] }}
                            </td>
                        </tr>
                        ...
                        <tr class="bg-white border-bottom-0 bg-white">
                            <td class="py-1"></td>
                        </tr>
                    </tbody>
...
const empresa = ref('');
const subtitle = 'Estado de Cuenta Individual del Cliente';
