<template>
    <div class="flex flex-column max-h-74vh">
        ...
        <div id="contenido-a-imprimir" class="flex-grow-1 overflow-y-auto max-h-2-rem">
            <GenericIndexReportGenerator :data="cuentasContables" :headers="columns" :title="empresa" subtitle="Productos" :filter="filter" />
        </div>
...
import { getProductosPdf } from '@/http/productos/productos-api'; // Asumo que esta es la función para obtener datos
...
const columns = ref([
    ...
    { title: 'ID', key: 'id_producto', summable: false, align: 'center', width: '5%'}, 
    { title: 'Precio Venta', key: 'precio_venta', summable: false, align: 'right', width: '10%' },
    { title: 'Estado', key: 'fecha_alta', summable: false, align: 'left', width: '10%' } 
...
const fetchData = async (filters = null) => {
    ...
    const response = await getProductosPdf({ desde: filters?.desde, hasta: filters?.hasta });
