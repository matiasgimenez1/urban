...
import { getProductos } from '@/http/productos/productos-api';
...
const onRowClick = (event) => {
    ...
    router.push({ name: 'producto', params: { id: productoId } });
};

const onADD = (event) => {
    router.push({ name: 'producto-insert' });
};
...
<template>
    <div class="grid my-2">
        <div class="col-12">
            ...
            <h5 class="text-blue-500 text-2xl text-center">Artículos, Bienes y Servicios</h5>
            ...
            >
                <Column field="attributes.nombre" header="Nombre" headerClass="text-white bg-blue-500 text-sm" bodyClass=" text-black text-sm"></Column>
                <Column field="attributes.precio_venta" header="Precio de Venta" headerClass="text-white bg-blue-500 text-sm" bodyClass=" text-black text-sm" class="text-right">
                    <template #header>
                        <div class="flex-1 text-right"></div>
                    </template>
                    <template #body="slotProps">
                        {{ formatNumber(slotProps.data.attributes.precio_venta || 0) }}
                    </template>
                </Column>
                <Column field="" header="" headerClass="text-white bg-blue-500 text-sm shadow" bodyClass=" text-black text-sm">
                        ...
                        <template #body="slotProps">
                            <div class="text-center text-sm">
                                {{
                                    slotProps.data.attributes.estado == 'A'
                                        ? 'ACTIVO'
                                        : slotProps.data.attributes.estado == 'I'
                                        ? 'INACTIVO' 
                                        : 'DESCONOCIDO'
                                }}
                            </div>
